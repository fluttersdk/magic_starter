...
- **Guest claim: `MagicStarterGuestClaim` and `GuestClaimOutcome`.** With `guest_auth` on, a guest sign-in records the guest's token and id in `Vault` (`guest_claim_token`, `guest_claim_user`), and the next sign-in or restore of a real account posts `POST /auth/guest/claim` with the guest token and the target's bearer, unawaited so it never blocks `Auth.login` or `Magic.init`. A 2xx (claimed) or a 404 or 422 (refused) drops the record, anything else keeps it, and a promoted guest (same id) settles with no request; a 404 is final because a backend without the claim route would otherwise receive the guest's live token on every restore. Every sign-out deletes the record before `Auth.logout()` returns and ends the session a claim in flight belongs to, so a late answer settles as `none` and never deletes a guest recorded after it. Concurrent `claimIfPending()` callers within one session share one in-flight claim. The claim goes out on a bare `DioNetworkDriver` built from `network.drivers.api`, not the facade driver `magic_devtools` records bodies from. Settled outcomes reach the new `MagicStarter.useGuestClaimed()` / `MagicStarterManager.onGuestClaimed` hook, also a `MagicStarter.bootstrap()` argument. (`lib/src/http/magic_starter_guest_claim.dart`, `lib/src/providers/magic_starter_service_provider.dart`, `lib/src/magic_starter_manager.dart`, `lib/src/facades/magic_starter.dart`)
...
- **`MagicStarter.useLogin()` / `MagicStarterManager.onLogin`**, a hook called after a fresh sign-in (password, two-factor challenge, social, guest, phone OTP), settable individually or through `MagicStarter.bootstrap()`. Deliberately not called on a cold-boot restore of a stored session or a team switch: `MagicStarterServiceProvider` listens for magic's `AuthLogin` event rather than `AuthRestored`, unawaited and logged on failure. (`lib/src/magic_starter_manager.dart`, `lib/src/facades/magic_starter.dart`, `lib/src/providers/magic_starter_service_provider.dart`)

- **A signed-in user's saved locale is now applied to the running app automatically**, on sign-in and on `Auth.restore()`, gated by the new `magic_starter.localization.apply_user_locale` config key (default `true`). Cooperates with a profile save's own language switch rather than racing it, so one save still issues exactly one `Lang.setLocale()` call. (`lib/src/providers/magic_starter_service_provider.dart`, `lib/src/configuration/magic_starter_config.dart`)
